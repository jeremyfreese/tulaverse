...
gallery <- (p_scatter | p_bar | p_box) /
  ...
  plot_annotation(
    title    = "theme_tula(): Stata-inspired ggplot2 defaults",
    subtitle = "Six geom types with theme_tula() + scale_color_tula() / scale_fill_tula()",
    theme    = theme_tula(base_size = bs)
  )

ggsave("theme_tula_gallery.png", gallery,
       width = 13, height = 8, dpi = 150, bg = "white")

message("Wrote theme_tula_gallery.png")
